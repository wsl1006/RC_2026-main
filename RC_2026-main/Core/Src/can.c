...
/* USER CODE BEGIN 0 */
/*扩展帧*/
#define CAN_EXT_ID_WORD(id)       (((uint32_t)(id) << 3U) | CAN_ID_EXT) //左移3位并扩展帧ID
#define CAN_EXT_FILTER_HIGH(word) ((uint16_t)((uint32_t)(word) >> 16))  //高16位
#define CAN_EXT_FILTER_LOW(word)  ((uint16_t)((uint32_t)(word) & 0xFFFFU))//低16位
/*标准帧*/
#define CAN_STD_ID_16BIT(id) ((uint16_t)((uint16_t)(id) << 5U)) //直接高五位
/* USER CODE END 0 */
...
void MX_CAN2_Init(void)
{
  ...
  CAN_FilterConfig.FilterFIFOAssignment = CAN_FILTER_FIFO0;       // 用哪个FIFO的信箱（FIFO1没有读回调，全部收进FIFO0）
  //0x000-0x7FF 11位标准帧
  //0x00000000-0x1FFFFFFF 29位扩展帧
